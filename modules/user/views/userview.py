...
from modules.user.serializers.userserializer import UserSerializer, ListUserSerializer
from backend.utils import helper as HELPER
...
class CustomAuthToken(ObtainAuthToken):
    def post(self, request, *args, **kwargs):
        ...
        """
        header_medium, header_portal = HELPER.get_headers(request.META)
        logger.info(f"USER LOGIN API CALL: Medium: {header_medium}, Portal: {header_portal}")
...
class UserCreateAPIView(viewsets.ViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        """
        header_medium, header_portal = HELPER.get_headers(request.META)
        logger.info(f"USER CREATE API CALL: Medium: {header_medium}, Portal: {header_portal}")
        data = request.data
        try:
...
class UserAPIView(viewsets.ViewSet):
    ...
    def list(self, request):
        """ User list call """
        header_medium, header_portal = HELPER.get_headers(request.META)
        logger.info(f"USER LIST API CALL: Medium: {header_medium}, Portal: {header_portal}")
    ...
    def update_user(self, request, format=None):
        ...
        """
        header_medium, header_portal = HELPER.get_headers(request.META)
        logger.info(f"USER UPDATE API CALL: Medium: {header_medium}, Portal: {header_portal}")
        data = request.data
        try:
            # Checking if create user request is made by super user or manager only
            if not request.user.is_superuser and request.user.role != SETTINGS.MANAGER_ROLE:
    ...
    def update_profile(self, request, format=None):
        ...
        """
        header_medium, header_portal = HELPER.get_headers(request.META)
        logger.info(f"USER UPDATE PROFILE API CALL: Medium: {header_medium}, Portal: {header_portal}")
        data = request.data
        try:
            # Getting user object that is to be updated
    ...
    def delete(self, request):
        ...
        header_medium, header_portal = HELPER.get_headers(request.META)
        logger.info(f"USER DELETE API CALL: Medium: {header_medium}, Portal: {header_portal}")
        try:
    ...
    def state(self, request, pk=None):
        ...
        header_medium, header_portal = HELPER.get_headers(request.META)
        logger.info(f"USER STATE CHANGE API CALL: Medium: {header_medium}, Portal: {header_portal}")
    ...
    def me(self, request):
        """ User me call """
        header_medium, header_portal = HELPER.get_headers(request.META)
        logger.info(f"USER ME API CALL: Medium: {header_medium}, Portal: {header_portal}")
        return Response({ 
            "status": "success",
    ...
    def retrieve(self, request, pk):
        """ Specific user call """
        header_medium, header_portal = HELPER.get_headers(request.META)
        logger.info(f"USER DETAIL API CALL: Medium: {header_medium}, Portal: {header_portal}")
        try:
    ...
    def change_password(self, request):
        ...
        """
        header_medium, header_portal = HELPER.get_headers(request.META)
        logger.info(f"USER CHANGE PASSWORD API CALL: Medium: {header_medium}, Portal: {header_portal}")
        data = request.data
        try:
            valid, message = True, ''
            # Password1 and password2 should be same
    ...
    def logout(self, request):
        """ User logout call """
        header_medium, header_portal = HELPER.get_headers(request.META)
        logger.info(f"USER LOGOUT API CALL: Medium: {header_medium}, Portal: {header_portal}")
        try:
